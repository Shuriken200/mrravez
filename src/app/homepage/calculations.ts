 ...
 * Fades in after "Hi!" starts fading, then fades out before about card
 */
...
/**
 * Calculate about card visibility
 */
export function calculateAboutVisibility(
	scrollProgress: number,
	isMobile: boolean
): SectionVisibility {
	const { entryStart, entryEnd, exitStart, exitEnd } = SCROLL_ZONES.about;
...
export function calculateAllVisibility(
	...
	const aboutBase = calculateAboutVisibility(scrollProgress, isMobile);
	...
	return {
		greeting,
		welcome,
		about: { ...aboutBase, opacity: aboutBase.opacity * jumpMultiplier },
...
export function calculateActiveSection(progress: number): number {
	if (progress < SECTION_THRESHOLDS.aboutToLinks) {
		return 0; // About
